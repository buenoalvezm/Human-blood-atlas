#### Title: Machine learning against healthy 
...
dir.create(paste0("results_disease_healthy/"), showWarnings = FALSE)
dir.create(paste0("results_disease_healthy/model/"), showWarnings = FALSE)
dir.create(paste0("results_disease_healthy/", class), showWarnings = FALSE)
set.seed(seed)
...
# Run binary ML for each disease against healthy
cat(paste0("Starting binary (healthy) runs for seed ",seed))
t0 <-  Sys.time()
healthy_ml_results_splits <-
  future_map(include_diseases,
             \(disease)
             ...
             disease_against_healthy(
               ...
               path_save =  paste0("results_disease_healthy/", class),
               seed = seed
             ),
             
             .options = furrr_options(seed = TRUE)
  ) |>
  set_names(include_diseases)
t1 <-  Sys.time()
gc()

# Save output and record run time
saveRDS(healthy_ml_results_splits, paste("results_disease_healthy/model/", class, "_healthy_ml_results_splits_", seed, ".rds", sep = ""))
rm(healthy_ml_results_splits)
gc()
t1-t0
