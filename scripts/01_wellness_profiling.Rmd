...
lod_sample_dat|> 
  ...
  filter(!DAid %in% samples_low_det$DAid) |> 
...
wellness_meta <- 
  wellness_meta |> 
  filter(DAid %in% wellness_data$DAid)
...
dat |> 
  filter(subject == "1-4270") |> 
  arrange(-above_LOD_count)
...
plot_complete
ggsave(savepath_results("Publication-figures", "LOD_per_sample_names.pdf"), h = 4.5, w = 18) #12


weird_subjects <- c("1-3119", "1-3217", "1-3293", "1-3790")
...
plot_weird <- 
  ...
  mutate(subject = factor(subject, levels = subject_order$subject)) |>
  summarise(n = sum(n)) |> 
  mutate(Weird = ifelse(subject %in% weird_subjects, "Yes", "No")) |> 
  ggplot(aes(subject, n, fill = Weird)) +
  geom_col() +
  theme_hpa() + #, axis_x = F
  xlab("Subject") +
  theme(axis.ticks.x = element_blank(), 
...
plot_complete / plot_weird


weird_subject <- 
  wellness_meta |> filter(subject == "1-4270")

wellness_data |> 
  filter(DAid %in% weird_subject$DAid) |> 
  group_by(DAid) |> 
  count(above_LOD) |> 
  distinct(DAid)
...
colors <- colorRampPalette(brewer.pal(12, "Paired"))(94)
