...
olink_targets <- read_csv("data/other/overlap_olink_platforms.csv")
...
plot_id <- 
  ...
  mutate(rank = rank(-n, ties.method = "first"),
         y = 1) |> 
  ggplot(aes(y, Subject, fill = Subject)) +
...
plot_wellness <- 
  meta_wellness |> 
  mutate(Subject = factor(Subject, levels = wellness_ids),
...
plot_id | plot_wellness  

  ggsave(savepath("wellness_visit_timeline.pdf"), width = 10, height = 5)
...
data_wellness_detected <- 
  data_wellness |>
  filter(Assay %in% proteins_above_LOD_80$Assay)
```
...
ggsave(savepath_results("Publication-figures", "Fig1f.pdf"), h = 6, w = 6)
...
data_mixed_model <- 
  ...
  left_join(meta_wellness |> 
              ...
              select(DAid, subject = Subject, visit = Visit, age = Age, sex = Sex), by = "DAid")
...
ggsave(savepath_results("Publication-figures", "MM-variance-explained.pdf"), h = 4, w = 18)
```
