...
# Previous version - only one threshold
# de_plot <- 
#   combined_de |> 
...
res_enrichment <- 
  ...
  map_df(controls, function(control) {
    ...
    map_df(current_diseases, function(disease) {
...
res_enrichment |> 
  write_excel_csv2(savepath_results("Enrichment", "Enrichment_all_diseases.csv"),                   col_names = T)
...
res_enrichment |> 
  count(Disease, Control) |> 
  arrange(-n)


ann <- 
  res_enrichment |> 
  distinct(Disease, Control) |> 
  mutate(rows = paste(Disease, Control, sep = "_")) |> 
  left_join(meta_disease |> 
              distinct(Disease, Class), by = "Disease") |> 
  select(-Disease) |> 
...
library(viridis)
res_enrichment |> 
  filter(Description %in% top_30_pathways$Description) |> 
  select(Disease, Control, p.adjust, Description) |> 
  mutate(p.adjust = -log10(p.adjust)) |> 
  pivot_wider(names_from = c(Disease, Control), values_from = p.adjust) |>
  column_to_rownames("Description") |> 
  pheatmap(show_colnames = F,
           annotation_col = ann,
           color = viridis(100, option = "magma"),
           annotation_colors = list(Class = pal_class,
                                    Control = pal_controls)) |> 
  as.ggplot()

ggsave(savepath_results("Manuscript-figures", "GO_enrichment.pdf"),
       height = 6,
       width = 18)
