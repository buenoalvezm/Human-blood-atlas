...
# Other data
ukb_associations <-
  read_csv("data/UkbUnivariateDiseaseResults/UkbUnivariateDiseaseAssociations.csv")
gadd_2024 <- import_df("data/UKB/mortality_Gadd_2024.xlsx")
you_2024 <- import_df("data/UKB/all_cause_mortality_You_2024.xlsx")
olink_uniprot_mapping <-
  read_tsv("../Human-disease-blood-atlas/data/olink_uniprot_mapping.tsv")
```
...
Identify differentially abundant proteins across diseases using different sets of controls.
...
write_tsv(combined_de,
          savepath_data("Supplementary-files", "Data_S6.tsv"))
```

# Visualization

Summarize and display patterns in differential expression to highlight disease-specific and shared protein signatures.

## Number of significant proteins across diseases and comparisons

```{r}
combined_de_ext <-
  combined_de |>
  mutate(
    Significance_FC1_pval05 = case_when(
      logFC > 1 & adj.P.Val < 0.05 ~ "significant up",
      logFC < -1 &
        adj.P.Val < 0.05 ~ "significant down",
      T ~ "not significant"
    ),
    Significance_FC05_pval005 = case_when(
      logFC > 0.5 & adj.P.Val < 0.005 ~ "significant up",
      logFC < -0.5 &
        adj.P.Val < 0.005 ~ "significant down",
      T ~ "not significant"
    ),
    Significance_FC1_pval005 = case_when(
      logFC > 1 & adj.P.Val < 0.005 ~ "significant up",
      logFC < -1 &
        adj.P.Val < 0.005 ~ "significant down",
      T ~ "not significant"
    ),
    Significance_FC1_pval0005 = case_when(
      logFC > 1 & adj.P.Val < 0.0005 ~ "significant up",
      logFC < -1 &
        adj.P.Val < 0.0005 ~ "significant down",
      T ~ "not significant"
    )
  ) |>
...
plot_de <-
  ...
  pivot_longer(
    cols = c(
      ...
      "Significance_FC1_pval05",
      "Significance_FC05_pval005",
      "Significance_FC1_pval005",
      "Significance_FC1_pval0005"
    ),
    names_to = "Threshold",
  ...
  count(Significance) |>
  mutate(
    Disease = factor(Disease, levels = rev(disease_class_order)),
...
ggsave(
  savepath_results("Manuscript-figures", "DE_thresholds.pdf"),
  h = 13,
  w = 19
)
```
...
ggsave(
  savepath_results("Manuscript-figures", "markers_class.pdf"),
  height = 2.5,
  width = 12
)
```
...
ggsave(
  savepath_results("Manuscript-figures", "Network_de_overlap.pdf"),
  h = 8,
  w = 8
)
```
...
Assess enrichment of biological pathways among differentially expressed proteins.
...
write_excel_csv(go_bp_enrichment,
                savepath_data("Supplementary-files", "Data_S7.csv"))
```
...
barplot
ggsave(
  savepath_results("Manuscript-figures", "enrichment_barplot_final.pdf"),
  h = 10,
  w = 7
)
```
...
theme_matrix <-
  ...
  left_join(go_reduced |>
              filter(parentTerm %in% keep_parent_terms$parentTerm),
            by = c("ID" = "go")) |>
...
ggsave(
  savepath_results("Manuscript-figures", "enrichment_heatmap_final.pdf"),
  h = 12,
  w = 12
)
```
...
Characterize proteins associated with multiple diseases.


## Identify proteins significant for many diseases
...
ggsave(
  savepath_results("Manuscript-figures", "general_disease_markers.pdf"),
  height = 3,
  width = 10
)
```
...
anova_res <-
  read_csv2(savepath_data("Supplementary-files", "Data_S5.csv"))

# Extract ANOVA proteins for age, sex, bmi
anova_dat <-
  anova_res |>
...
plot_age_sex_bmi <-
  ...
  distinct(Assay) |>
  mutate(
    Sex = ifelse(Assay %in% sex_proteins, "Sex", "No"),
...
ggsave(
  savepath_results("Manuscript-figures", "general_disease_markers_v2.pdf"),
  height = 10,
  width = 5
)
```
