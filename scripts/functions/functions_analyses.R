  ...
  if (isSingular(mixed_model, tol = 1e-4)) {
    message(
      paste(
        "Protein",
        protein,
        ": Singular fit detected. Setting random variance explained to 0."
      )
    )
  ...
  function(variable_predict,
    ...
    split_train <-
      ...
      mutate(Variable = case_when(
        Variable == variable_case ~ paste0("1_", Variable),
    ...
    final_glmnet_fit <-
      last_fit(glmnet_wflow, variable_split, metrics = eval_metrics)
