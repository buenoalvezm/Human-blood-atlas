...
#   add_variables(outcomes = NPX, 
#                 predictors = c(age, sex, subject)) %>% 
#   add_model(lmer_spec, formula = NPX ~ age + sex + (1|subject))

# lmer_model <- fit(lmer_wflow, data = df)
# 
# 
# 
# 
...
# 
# # Get tidy model results
# tidy_model <- broom::tidy(mixed_model)
# 
# # Add Eta-squared to the result
# tidy_anova <- 
#   tidy_anova |> 
