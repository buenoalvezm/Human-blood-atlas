...
dat_wellness <-
  readRDS(savepath_data("MM_test", "data_mm_Wellness.rds"))
...
test_interaction_wellness <-
  map_df(unique(dat_wellness$Assay),
         ~ test_mm_interaction(dat_wellness, .x))
saveRDS(
  test_interaction_wellness,
  savepath_data("MM_test", "test_interaction_wellness.rds")
)

test_interaction_bamse <-
  map_df(unique(dat_bamse$Assay), ~ test_mm_interaction(dat_bamse, .x))
saveRDS(test_interaction_bamse,
        savepath_data("MM_test", "test_interaction_bamse.rds"))


# Run random slope tests
test_slopes_wellness <-
  map_df(unique(dat_wellness$Assay),
         ~ test_mm_slopes(dat_wellness, .x))
saveRDS(test_slopes_wellness,
        savepath_data("MM_test", "test_slopes_wellness.rds"))

test_slopes_bamse <-
  map_df(unique(dat_bamse$Assay), ~ test_mm_slopes(dat_bamse, .x))
saveRDS(test_slopes_bamse,
        savepath_data("MM_test", "test_slopes_bamse.rds"))
...
summary_signficance_interaction <-
  ...
  bind_rows(
    summary_signficance(
      test_df = test_interaction_bamse,
      test = "Interaction",
      dataset = "BAMSE"
    )
  )
...
write_tsv(
  summary_signficance_interaction,
  savepath_data("MM_test", "summary_signficance_interaction.tsv")
)
...
write_tsv(
  summary_singular_interaction,
  savepath_data("MM_test", "summary_singular_interaction.tsv")
)
...
summary_signficance_slopes <-
  summary_signficance(test_df  = test_slopes_wellness,
                      test = "Slopes",
                      dataset = "Wellness") |>
  bind_rows(summary_signficance(
    test_df = test_slopes_bamse,
    test = "Slopes",
    dataset = "BAMSE"
  ))
...
write_tsv(
  summary_signficance_slopes,
  savepath_data("MM_test", "summary_signficance_slopes.tsv")
)
...
summary_singular_slopes <-
  summary_singular(test_df  = test_slopes_wellness,
                   test = "Slopes",
                   dataset = "Wellness") |>
  bind_rows(summary_singular(
    test_df = test_slopes_bamse,
    test = "Slopes",
    dataset = "BAMSE"
  ))

write_tsv(summary_singular_slopes,
          savepath_data("MM_test", "summary_singular_slopes.tsv"))
